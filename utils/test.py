...
csv_files = sorted([f for f in os.listdir(test_data_dir) if f.endswith(".csv")])

print("Reading CSVs from:", test_data_dir)
...
try:
    for file in csv_files:
        ...
        for frame_id in sorted(df["Frame"].unique()):
            ...
            db = DBSCAN(eps=1.5, min_samples=1).fit(positions)
            labels = db.labels_
