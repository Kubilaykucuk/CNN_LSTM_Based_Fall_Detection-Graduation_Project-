...
This script reads radar data streamed from MATLAB, processes it with a CNN-LSTM model
to classify movement types, clusters object positions, and sends results to the ThingsBoard cloud
via MQTT in real time.
"""
...
MATLAB_PATH = os.getenv("MATLAB_SCRIPT", "path_to_radar_code.m")
...
ACCESS_TOKEN = "5bs494xjaob1whkr86bu"
...
# === Launch MATLAB Radar ===
matlab_command = ["matlab", "-batch", f"run('{MATLAB_PATH}')"]
...
try:
    for line in proc.stdout:
        ...
        for frame_id in sorted(df["Frame"].unique()):
            ...
            if track_data:
                json_msg = json.dumps(track_data)
                print(f"📤 Sending frame with {len(track_data)} objects")
